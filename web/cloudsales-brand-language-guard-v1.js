...
function text(sel,value){const el=q(sel);if(el&&value!=null)el.textContent=value}
function html(sel,value){const el=q(sel);if(el&&value!=null)el.innerHTML=value}
function meta(name,value){const el=q(`meta[name="${name}"]`);if(el)el.setAttribute('content',value)}

function installStyle(){
  let s=document.getElementById('cloudsales-canonical-brand-v1');
  if(!s){s=document.createElement('style');s.id='cloudsales-canonical-brand-v1';document.head.appendChild(s)}
  s.textContent=`
...
html,body{background:${BRAND.canvas}!important;color:${BRAND.white}!important}
body{background-image:radial-gradient(920px 520px at 50% -180px,${BRAND.purple} 0,rgba(45,10,74,.62) 28%,transparent 72%)!important}
.nav{background:rgba(8,7,13,.94)!important;border-bottom:1px solid rgba(249,85,182,.12)!important}
.brand img{height:39px!important;width:auto!important;max-width:210px!important;object-fit:contain!important;background:transparent!important}
.hero h1,.section h2,.csStoryTitle,.csBmp h3{color:${BRAND.white}!important}
.grad,.hero h1 .grad,.hero h1 span.grad{background:linear-gradient(90deg,${BRAND.white} 0%,#F7D7EC 34%,${BRAND.pink} 100%)!important;-webkit-background-clip:text!important;background-clip:text!important;-webkit-text-fill-color:transparent!important;color:transparent!important;opacity:1!important}
.hero p,.lead,.card p,.faq p,.micro,.csStoryLead,.csStoryCard p,.csBmpIntro{color:${BRAND.muted}!important}
.btn.primary,.csHeroTrial,.csFinalTrial,.csFooterCrmBtn{background:linear-gradient(135deg,${BRAND.pink} 0%,#E548C9 58%,${BRAND.violet} 100%)!important;border:0!important;box-shadow:0 14px 38px rgba(249,85,182,.27)!important;color:#fff!important}
.btn:not(.primary){background:#121019!important;border-color:#3B3442!important;color:${BRAND.white}!important}
.card,.crm,.faq details,.included,.mission,.csStoryCard,.csSaving{background:linear-gradient(180deg,#15121B,#0E0C13)!important;border-color:#39323F!important}
.csHookPanel,.csFinalBox,.csBmp{background:radial-gradient(520px 250px at 100% 0,rgba(249,85,182,.09),transparent 70%),linear-gradient(145deg,rgba(45,10,74,.54),#100D14)!important;border-color:rgba(249,85,182,.26)!important}
.csStoryKicker,.csBmpKicker,.csStoryNum{color:#F6A5D4!important}
.csGrowthIndex,.csBmpStep b{background:linear-gradient(135deg,${BRAND.pink},${BRAND.violet})!important}
...
.cs-crm-call{background:linear-gradient(90deg,rgba(249,85,182,.09),rgba(45,10,74,.34))!important;border-color:rgba(249,85,182,.23)!important}
.cs-crm-call strong{background:linear-gradient(90deg,${BRAND.white},#F8C9E5,${BRAND.pink})!important;-webkit-background-clip:text!important;background-clip:text!important;color:transparent!important}
@media(max-width:620px){.brand img{height:35px!important;max-width:174px!important}.navin{gap:10px!important}.hero h1{letter-spacing:-.055em!important}}
`;
}

function applySupported(lang){
  const c=COPY[lang];
  document.documentElement.dataset.csLanguageIntegrity='strict';
  document.documentElement.dataset.csBrandCanonical=VERSION;

  html('section.hero h1,.hero h1',c.hero);
  text('section.hero .eyebrow,.hero .eyebrow',c.eyebrow);
  const hero=q('section.hero,.hero');
  if(hero){
    const p=hero.querySelector('p'); if(p)p.textContent=c.lead;
    const m=hero.querySelector('.micro'); if(m)m.textContent=c.micro;
  }

  text('#cs-story-hook .csStoryKicker',c.hookKicker);
  text('#cs-story-hook .csStoryTitle',c.hookTitle);
  text('#cs-story-hook .csStoryLead',c.hookLead);
  text('#cs-story-setup .csStoryKicker',c.setupKicker);
  text('#cs-story-setup .csStoryTitle',c.setupTitle);
  text('#cs-story-setup .csStoryLead',c.setupLead);
  text('#cs-story-cloudy .csStoryKicker',c.cloudyKicker);
  text('#cs-story-cloudy .csStoryTitle',c.cloudyTitle);
  text('#cs-story-cloudy .csStoryLead',c.cloudyLead);
  text('#cs-story-agents .csStoryKicker',c.agentsKicker);
  text('#cs-story-agents .csStoryTitle',c.agentsTitle);
  text('#cs-story-agents .csStoryLead',c.agentsLead);
  text('#cs-story-app .csStoryKicker',c.appKicker);
  text('#cs-story-app .csStoryTitle',c.appTitle);
  text('#cs-story-app .csStoryLead',c.appLead);
  text('#cs-story-app .csCalm',c.calm);
  text('#cs-story-final .csStoryKicker',c.finalKicker);
  text('#cs-story-final .csStoryTitle',c.finalTitle);
  text('#cs-story-final .csStoryLead',c.finalLead);

  text('#business-management-platform .csBmpKicker',c.bmpKicker);
  text('#business-management-platform h3',c.bmpTitle);
  text('#business-management-platform .csBmpIntro',c.bmpIntro);
  text('#business-management-platform .csBmpQuality',c.bmpQuality);
  document.querySelectorAll('.cs-crm-call strong').forEach(el=>el.textContent=c.crmCall);

  if(lang==='es'){
    document.querySelectorAll('*').forEach(el=>{
      if(el.children.length) return;
      const t=(el.textContent||'').trim();
      if(t==='PARTE 1 · SETUP + PROTECCIÓN')el.textContent='PARTE 1 · CONFIGURACIÓN + PROTECCIÓN';
      if(t==='Atención y troubleshooting')el.textContent='Atención y resolución de problemas';
      if(t==='Lead Quality')el.textContent='Calidad de prospectos';
    });
    document.title='CloudSales — IA trabajando por ti, mejores leads y control desde tu celular';
    meta('description','CloudSales opera sobre tu CRM con Cloudy y AgentCloud, ayuda a reducir junk leads, prioriza mejores oportunidades y te permite controlar tu operación desde la palma de tu mano.');
  }else{
    document.title='CloudSales — AI working for you, better leads and mobile control';
    meta('description','CloudSales operates on top of your CRM with Cloudy and AgentCloud, helps reduce junk leads, prioritizes better opportunities, and lets you control your operation from the palm of your hand.');
  }
}

function enforceUnsupported(){
  document.documentElement.dataset.csLanguageIntegrity='strict-no-fallback';
  document.querySelectorAll('#cs-story-hook,#cs-story-setup,#cs-story-cloudy,#cs-story-growth,#cs-story-agents,#cs-story-app,#cs-story-final,#business-management-platform,.csStoryPricingNote,.csCrmMore,.cs-crm-call').forEach(el=>{
    el.hidden=true;
    el.setAttribute('data-cs-withheld-untranslated','1');
  });
}
...
function apply(){
  ...
  installStyle();
  const l=locale();
  document.querySelectorAll('[data-cs-withheld-untranslated="1"]').forEach(el=>{el.hidden=false;el.removeAttribute('data-cs-withheld-untranslated')});
  if(l==='es'||l==='en') applySupported(l); else enforceUnsupported();
}
