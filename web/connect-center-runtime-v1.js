...
const tabs={
  ...
  other:{
    ...
    description:'Conecta WhatsApp, SMS y productividad para que CloudSales pueda operar con el contexto y los permisos autorizados de tu negocio.',
    items:[
      ['telnyx','Telnyx · WhatsApp / SMS','telnyx','telnyx.com'],
...
function card(x){
  ...
  const cardClass=telnyxPending?'isPending':connected?'isConnected':'';
  const status=telnyxPending?'<span class="csConnectStatus pending">API conectada</span>':connected?'<span class="csConnectStatus">✓ Conectado</span>':'';
  const subtitle=connected?esc(c.external_account_name||'Cuenta conectada'):provider==='telnyx'?'SMS · WhatsApp · Voice':'Integración CloudSales';
  let action='';
  if(provider==='telnyx'&&connected)action=`<button class="csConnectAction" data-provider="telnyx">${telnyxConfigured?'Administrar':'Terminar configuración'}</button>`;
  else if(!connected)action=available?`<button class="csConnectAction" data-provider="${esc(provider)}">Conectar</button>`:'<span class="csConnectUnavailable">Próximamente</span>';
...
function openTelnyx(){
  closeTelnyx();
  const c=connection('telnyx'),configured=!!c?.metadata?.webhook_public_key_configured;
  const el=document.createElement('div');el.className='csTelnyxBackdrop';
  el.innerHTML=`<div class="csTelnyxModal" role="dialog" aria-modal="true" aria-label="Conectar Telnyx">
    <div class="csTelnyxHead"><div><div class="csConnectEyebrow">CLOUDSALES COMMUNICATIONS</div><h2>${c?'Administrar Telnyx':'Conectar Telnyx'}</h2><p>${c?'La API de Telnyx ya está conectada. Termina o actualiza la verificación de webhooks.':'Conecta SMS, WhatsApp y, después del E2E, voz dentro del Communications Engine.'}</p></div><button class="csTelnyxClose" aria-label="Cerrar">×</button></div>
    ...
    <div class="csTelnyxNote">La API key se guarda en Vault y no se vuelve a mostrar. CloudSales usa la Public Key únicamente para validar la firma Ed25519 de los webhooks. El proveedor permanece desactivado para tráfico de producción hasta pasar las pruebas E2E.</div>
    <div id="csTelnyxMsg" class="csTelnyxMsg"></div>
    <div class="csTelnyxActions">
      <button id="csTelnyxSave" class="primary">${c?'Guardar configuración':'Conectar Telnyx'}</button>
      <a href="https://telnyx.com/agent-signup.md" target="_blank" rel="noopener">Crear cuenta / API key</a>
  ...
  save.onclick=async()=>{
    ...
    try{
      ...
      msg.className='csTelnyxMsg ok';msg.textContent=publicKey?'Telnyx conectado y firma de webhooks configurada.':'Telnyx API conectada. Falta la Public Key para habilitar webhooks firmados.';
      setTimeout(()=>{closeTelnyx();render()},650);
...
async function connect(provider){
  ...
  if(provider==='telnyx'){
    try{if(!connection('telnyx')&&!(await ensureLegal('telnyx')))return;openTelnyx()}catch(e){alert(e?.message||String(e))}
    return;
  }
  try{
...
function account(){
  ...
  if(!c){
    ...
    c.innerHTML='<h3>Conectar</h3><p>Conecta redes sociales, CRM, WhatsApp, SMS, Telegram y Notion desde tu cuenta.</p><button class="btn primary csAccountConnect">Administrar conexiones</button>';
    cards.prepend(c);
  }
