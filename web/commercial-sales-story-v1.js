...
const COPY={
  es:{
    ...
    pricingLead:'Elige el nivel que encaja hoy; Cloudy te ayuda a crecer sin obligarte a comprar una torre de herramientas desde el primer día.',
    ...
    finalKicker:'ACTIVA CLOUDSALES EN TU NEGOCIO',
    finalTitle:'Activa CloudSales y pon a Cloudy a trabajar.',
    finalLead:'Conecta tu operación y deja que Cloudy organice, proteja y mejore el trabajo que autorizas.',
    activate:'ACTIVAR CLOUDSALES',
    plans:'VER PLANES'
  },
  en:{
    ...
    pricingLead:'Choose the level that fits today; Cloudy helps you grow without forcing you to buy a tower of tools on day one.',
    ...
    finalKicker:'ACTIVATE CLOUDSALES FOR YOUR BUSINESS',
    finalTitle:'Activate CloudSales and put Cloudy to work.',
    finalLead:'Connect your operation and let Cloudy organize, protect and improve the work you authorize.',
    activate:'ACTIVATE CLOUDSALES',
    plans:'SEE PLANS'
  }
};
...
const final=`<section class="csFinal" id="cs-story-final"><div class="wrap"><div class="csFinalBox"><div class="csStoryKicker">${x.finalKicker}</div><h2 class="csStoryTitle">${x.finalTitle}</h2><p class="csStoryLead">${x.finalLead}</p><div class="csFinalActions"><a class="btn primary csFinalActivation" href="#pricing">${x.activate}</a><a class="btn" href="#pricing">${x.plans}</a></div></div></div></section>`;
...
function enhancePricing(){const x=c(),pricing=findPricing();if(!pricing)return null;pricing.id='pricing';const plans=pricing.querySelector('.plans');let note=pricing.querySelector('.csStoryPricingNote');if(!note&&plans){note=document.createElement('div');note.className='csStoryPricingNote csActivationBridge';note.innerHTML=`<div class="csActivationChip">${x.activate}</div><div><strong>${x.pricingTitle}</strong><br>${x.pricingLead}</div>`;plans.insertAdjacentElement('beforebegin',note)}pricing.querySelectorAll('.plan').forEach(card=>{if(!card.querySelector('.csActivationMini')){const h=card.querySelector('h3');if(h){const m=document.createElement('div');m.className='csActivationMini';m.textContent=x.activate;h.insertAdjacentElement('afterend',m)}}const b=card.querySelector('.btn');if(b){b.dataset.csActivationButton='1';const label=(card.querySelector('h3')?.textContent||'').trim().toUpperCase();b.textContent=lang()==='es'?`ELEGIR ${label}`:`CHOOSE ${label}`}});return pricing}
function enhanceHero(){const x=c(),hero=findHero();if(!hero)return;const eye=hero.querySelector('.eyebrow');if(eye)eye.textContent=x.eyebrow;const h=hero.querySelector('h1');if(h)h.innerHTML=x.hero;const p=hero.querySelector('p');if(p)p.textContent=x.heroLead;const m=hero.querySelector('.micro');if(m)m.textContent=x.micro;const actions=hero.querySelector('.actions');if(actions&&!actions.querySelector('.csHeroActivation')){const a=document.createElement('a');a.className='btn primary csHeroActivation';a.href='#pricing';a.textContent=x.activate;actions.prepend(a)}else if(actions?.querySelector('.csHeroActivation'))actions.querySelector('.csHeroActivation').textContent=x.activate}
