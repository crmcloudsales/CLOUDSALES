(() => {
  ...
  function render() {
    ...
    if (!overlay) create();
    title.textContent = ios() ? 'Instala CloudSales en tu iPhone o iPad' : 'Instala CloudSales en tu dispositivo';

    if (ios()) {
      fallbackMode = true;
      body.innerHTML = `<p>En Safari toca Compartir y después <b>Agregar a pantalla de inicio</b>.</p><p class="install-note">Puedes entrar a CloudSales sin instalarla.</p>`;
      primary.disabled = false;
      primary.textContent = 'Entrar a CloudSales';
      return;
    }

    if (deferredPrompt) {
      fallbackMode = false;
      body.innerHTML = `<p>CloudSales está lista para instalarse como aplicación.</p>`;
      primary.disabled = false;
    ...
    fallbackMode = false;
    body.innerHTML = `<p>Comprobando si Chrome puede abrir la instalación nativa…</p>`;
    primary.disabled = true;
    primary.textContent = 'Preparando instalación…';

    clearTimeout(readyTimer);
    readyTimer = setTimeout(() => {
      if (deferredPrompt || !primary) return;
      fallbackMode = true;
      body.innerHTML = `<p>Chrome no entregó el instalador nativo en esta sesión.</p><p class="install-note">CloudSales seguirá disponible en el navegador y Chrome puede ofrecer “Instalar aplicación” desde su menú.</p>`;
      primary.disabled = false;
      primary.textContent = 'Entrar a CloudSales';
    }, 1800);
  }
  ...
  async function install() {
    if (standalone()) { close(); return; }
    if (fallbackMode || ios() || !deferredPrompt) { enterApp(); return; }
    primary.disabled = true;
    const prompt = deferredPrompt;
    deferredPrompt = null;
    try {
