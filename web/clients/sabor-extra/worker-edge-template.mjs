...
const FAVICON=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e8a62a"/><stop offset="1" stop-color="#8f2d1f"/></linearGradient></defs><rect width="64" height="64" rx="18" fill="#fff7e8"/><rect x="7" y="7" width="50" height="50" rx="16" fill="url(#g)"/><text x="32" y="40" text-anchor="middle" font-family="Arial,sans-serif" font-size="24" font-weight="900" fill="white">SE</text></svg>`;
...
function secHeaders(){return{"x-frame-options":"DENY","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin","permissions-policy":"camera=(), microphone=(), geolocation=(), payment=()","cross-origin-opener-policy":"same-origin-allow-popups","x-permitted-cross-domain-policies":"none","content-security-policy":"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://static.wixstatic.com; connect-src 'self' https://fkahaqprzgcimgyathqx.supabase.co; frame-src https://www.google.com; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests"}}
function page(){return new Response(HTML,{headers:{...secHeaders(),"content-type":"text/html; charset=utf-8","cache-control":"public,max-age=180,stale-while-revalidate=600"}})}
function favicon(){return new Response(FAVICON,{headers:{...secHeaders(),"content-type":"image/svg+xml;charset=utf-8","cache-control":"public,max-age=86400"}})}
export default{async fetch(req,env){
 const u=new URL(req.url),path=u.pathname.replace(/\/+$/,'')||'/';
 if(req.method==='GET'&&(path==='/favicon.svg'||path==='/favicon.ico'||path==='/apple-touch-icon.png'))return favicon();
 if(req.method==='GET'&&path==='/health')return json({ok:true,service:'sabor-extra-gateway',version:'edge-2',challenge:'pow-hmac+turnstile',turnstile:'cloudflare-managed',seo:'indexable'});
 ...
 if(req.method==='GET'&&path==='/challenge'){const id=clean(u.searchParams.get('id'),180);if(!id)return json({error:'id_required'},400);const nonce=crypto.randomUUID(),ts=Date.now(),sig=await hmac(env.CHALLENGE_SECRET,`${id}.${nonce}.${ts}`);return json({nonce,ts,sig,prefix:'000'})}
 if(req.method==='GET'||req.method==='HEAD')return req.method==='HEAD'?new Response(null,{status:200,headers:{...secHeaders(),'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=180'}}):page();
 ...
 let score=70;const reasons=['edge_pow_hmac','cloudflare_turnstile'];if(em)score+=10;if(honey){score=0;reasons.push('honeypot')}
