...
const GATE_KEY="4bcb1863-87c9-44b9-85c6-4cd7bbc2a213";
const ORIGIN="https://saborextra.cloudsales.app/";
const INTAKE="https://fkahaqprzgcimgyathqx.supabase.co/functions/v1/lead-intake";
const SITEKEY="__TURNSTILE_SITEKEY__";
const FAVICON=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e8a62a"/><stop offset="1" stop-color="#8f2d1f"/></linearGradient></defs><rect width="64" height="64" rx="18" fill="#fff7e8"/><path d="M13 33c0-13 8-22 20-22 10 0 18 6 18 15 0 6-4 10-10 10H29c-3 0-5 2-5 5 0 5 4 8 10 8 6 0 11-2 15-6-2 9-9 14-19 14-11 0-19-7-19-17 0-3 1-5 2-7z" fill="url(#g)"/><path d="M24 22c5-4 12-4 17 0M22 29c6-3 13-3 19 0" fill="none" stroke="#fff" stroke-width="4" stroke-linecap="round"/></svg>`;
...
function secHeaders(){return{"x-frame-options":"DENY","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin","permissions-policy":"camera=(), microphone=(), geolocation=(), payment=()","cross-origin-opener-policy":"same-origin-allow-popups","x-permitted-cross-domain-policies":"none"}}
function page(){return new Response(HTML,{headers:{...secHeaders(),"content-type":"text/html; charset=utf-8","cache-control":"public,max-age=120,stale-while-revalidate=300","content-security-policy":"default-src 'self'; script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://static.wixstatic.com; connect-src 'self' https://challenges.cloudflare.com https://fkahaqprzgcimgyathqx.supabase.co; frame-src https://challenges.cloudflare.com https://www.google.com; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests"}})}
export default{async fetch(req,env){
 const u=new URL(req.url),path=u.pathname.replace(/\/+$/,'')||'/';
 if(req.method==='GET'&&path==='/health')return json({ok:true,service:'sabor-extra-gateway',version:'edge-2',challenge:'pow-hmac+turnstile',turnstile:'cloudflare-managed',seo:'indexable',favicon:'active'});
 if(req.method==='GET'&&path==='/favicon.svg')return new Response(FAVICON,{headers:{...secHeaders(),'content-type':'image/svg+xml;charset=utf-8','cache-control':'public,max-age=604800,immutable'}});
 if(req.method==='GET'&&path==='/favicon.ico')return Response.redirect(`${u.origin}/favicon.svg`,301);
 if(req.method==='GET'&&path==='/robots.txt')return new Response('User-agent: *\nAllow: /\nSitemap: https://saborextra.cloudsales.app/sitemap.xml\n',{headers:{'content-type':'text/plain;charset=utf-8','cache-control':'public,max-age=3600','x-content-type-options':'nosniff'}});
 if(req.method==='GET'&&path==='/sitemap.xml')return new Response('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://saborextra.cloudsales.app/</loc><lastmod>2026-09-01</lastmod><changefreq>weekly</changefreq><priority>1.0</priority></url></urlset>',{headers:{'content-type':'application/xml;charset=utf-8','cache-control':'public,max-age=3600'}});
 ...
 if(req.method==='GET'||req.method==='HEAD')return req.method==='HEAD'?new Response(null,{status:200,headers:{...secHeaders(),'content-type':'text/html; charset=utf-8','cache-control':'public,max-age=120'}}):page();
 ...
 const first=clean(b.first_name,120),last=clean(b.last_name,120),em=clean(b.email,320).toLowerCase(),phone=clean(b.phone,40).replace(/[^0-9+]/g,''),interest=clean(b.interest,100),message=clean(b.message,1500);
 if(first.length<2||phone.replace(/\D/g,'').length<8||!validEmail(em))return json({message:'Revisa nombre, teléfono y email.'},422);
 let score=35;const reasons=['edge_pow_hmac','cloudflare_turnstile'];score+=35;if(em)score+=10;const dwell=Date.now()-Number(b.started_at||Date.now());if(dwell>=1800)score+=5;else reasons.push('fast_submit');if(honey){score=0;reasons.push('honeypot')}
 const att=b.attribution||{};
 const payload={gate_key:GATE_KEY,idempotency_key:id,origin_url:ORIGIN,form_id:'sabor-extra-contact',form_name:'Sabor Extra · Contacto',form_answers:{interest,message},contact:{first_name:first,last_name:last,email:em,phone},attribution:{source_provider:clean(att.utm_source||'direct',80),campaign_id:clean(att.utm_campaign||'sabor-extra-main',180),ad_group_id:clean(att.utm_content||interest,180),ad_id:'',landing_url:ORIGIN,referrer:req.headers.get('Referer')||'',fbclid:clean(att.fbclid,300),gclid:clean(att.gclid,300)},security:{quality_score:Math.max(0,Math.min(100,score)),turnstile:turnstileOk,honeypot:honey,ip_hash:await sha(ip||'unknown'),user_agent_hash:await sha(ua||'unknown'),reasons}};
