...
function page(){return new Response(HTML,{headers:{"content-type":"text/html; charset=utf-8","cache-control":"public,max-age=120","x-frame-options":"DENY","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin","permissions-policy":"camera=(), microphone=(), geolocation=(), payment=()","cross-origin-opener-policy":"same-origin-allow-popups","x-permitted-cross-domain-policies":"none","content-security-policy":"default-src 'self'; script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://challenges.cloudflare.com https://fkahaqprzgcimgyathqx.supabase.co; frame-src https://challenges.cloudflare.com; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests"}})}
export default{async fetch(req,env){
 const u=new URL(req.url),path=u.pathname.replace(/\/+$/,'')||'/';
 if(req.method==='GET'&&path==='/health')return json({ok:true,service:'senzik-residences-gateway',version:'edge-1',challenge:'pow-hmac+turnstile',turnstile:'cloudflare-managed',seo:'indexable'});
 ...
 const first=clean(b.first_name,120),last=clean(b.last_name,120),em=clean(b.email,320).toLowerCase(),phone=clean(b.phone,40).replace(/[^0-9+]/g,''),region=clean(b.region,100),interest=clean(b.interest,100),message=clean(b.message,1500);
 if(first.length<2||phone.replace(/\D/g,'').length<8||!validEmail(em))return json({message:'Revisa nombre, teléfono y email.'},422);
 let score=70,reasons=['edge_pow_hmac','cloudflare_turnstile'];if(em)score+=10;const dwell=Date.now()-Number(b.started_at||Date.now());if(dwell>=1800)score+=5;else reasons.push('fast_submit');if(honey){score=0;reasons.push('honeypot')}
 const att=b.attribution||{};
 const payload={gate_key:GATE_KEY,idempotency_key:id,origin_url:ORIGIN,form_id:'senzik-residences-main',form_name:'Senzik Residences · Solicitud de información',form_answers:{region,interest,message},contact:{first_name:first,last_name:last,email:em,phone},attribution:{source_provider:clean(att.utm_source||'direct',80),campaign_id:clean(att.utm_campaign||'senzik-main',180),ad_group_id:clean(att.utm_content||interest,180),ad_id:'',landing_url:ORIGIN,referrer:req.headers.get('Referer')||'',fbclid:clean(att.fbclid,300),gclid:clean(att.gclid,300)},security:{quality_score:Math.max(0,Math.min(100,score)),turnstile:turnstileOk,honeypot:honey,ip_hash:await sha(ip||'unknown'),user_agent_hash:await sha(ua||'unknown'),reasons}};
