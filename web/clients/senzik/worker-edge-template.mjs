...
export default {
  async fetch(req,env) {
    ...
    if(req.method==='GET'&&path==='/health')return json({ok:true,service:'senzik-gateway-v1',version:'edge-3',security:'turnstile+honeypot+server-validation',whatsapp_gate:'lead_required'});
    ...
    const first=clean(b.first_name,120),last=clean(b.last_name,120),email=clean(b.email,320).toLowerCase(),phone=clean(b.phone,40).replace(/[^0-9+]/g,''),delivery=clean(b.delivery_timeline||b.interest,80),honeypot=clean(b.website,300)!=='';
    if(honeypot)return json({status:'accepted'},200);
    const allowedDelivery=new Set(['Entrega inmediata','3 a 6 meses','6 meses o más']);
    if(first.length<2||last.length<2||!emailOk(email)||phone.replace(/\D/g,'').length<8||!allowedDelivery.has(delivery))return json({message:'Completa nombre, apellido, email, teléfono/WhatsApp y plazo de entrega.'},422);
