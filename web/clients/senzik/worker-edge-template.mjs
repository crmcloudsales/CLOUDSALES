...
export default {
  async fetch(req,env) {
    ...
    const first=clean(b.first_name,120),last=clean(b.last_name,120),email=clean(b.email,320).toLowerCase(),phone=clean(b.phone,40).replace(/[^0-9+]/g,''),region=clean(b.region,100),interest=clean(b.interest,120),message=clean(b.message,1500),honeypot=clean(b.website,300)!=='';
    if(honeypot)return json({status:'accepted'},200);
    if(first.length<2||phone.replace(/\D/g,'').length<8||!emailOk(email))return json({message:'Revisa nombre, teléfono y email.'},422);
    ...
    const payload={gate_key:GATE_KEY,idempotency_key:clean(b.idempotency_key,180)||crypto.randomUUID(),origin_url:ORIGIN+'/',form_id:'senzik-residences-main',form_name:'Senzik Residences - Solicitud de información',form_answers:{region,interest,message},contact:{first_name:first,last_name:last,email,phone},attribution:{source_provider:clean(att.utm_source||'direct',80),campaign_id:clean(att.utm_campaign||'senzik-main',180),ad_group_id:clean(att.utm_content||interest,180),ad_id:'',landing_url:ORIGIN+'/',referrer:req.headers.get('Referer')||'',fbclid:clean(att.fbclid,300),gclid:clean(att.gclid,300)},security:{quality_score:85,turnstile:true,honeypot:false,reasons:['cloudflare_turnstile','server_validation']}};
