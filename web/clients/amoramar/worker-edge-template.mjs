...
const GATE_KEY="85035947-e94e-4605-a619-59915c1e7294";
const ORIGIN="https://amoramar.cloudsales.app/";
...
export default{async fetch(req,env){
 ...
 if(req.method==="GET"&&u.pathname==="/health")return json({ok:true,service:"amoramar-lead-gateway",version:"edge-1",challenge:"pow",inventory:"cloudsales"});
 ...
 if(req.method==="GET")return new Response(PAGE,{headers:{"content-type":"text/html; charset=utf-8","cache-control":"public,max-age=120","x-frame-options":"DENY","x-content-type-options":"nosniff","referrer-policy":"strict-origin-when-cross-origin","permissions-policy":"camera=(), microphone=(), geolocation=()","content-security-policy":"default-src 'self' https:; img-src 'self' https: data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self' https://fkahaqprzgcimgyathqx.supabase.co; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"}});
 ...
 const att=b.attribution||{},interest=String(b.interest||"").slice(0,180),property=String(b.property||"").slice(0,220),message=String(b.message||"").slice(0,1500);
 const payload={gate_key:GATE_KEY,idempotency_key:id,origin_url:ORIGIN,form_id:"amoramar-public-landing",form_name:"AMORAMAR Riviera Maya",form_answers:{interest,property,message},contact:{first_name:first,last_name:last,email:em,phone},attribution:{source_provider:String(att.utm_source||"direct").slice(0,80),campaign_id:String(att.utm_campaign||"amoramar-main").slice(0,180),ad_group_id:String(property||att.utm_content||"").slice(0,180),ad_id:String(att.ad_id||"").slice(0,180),landing_url:ORIGIN,referrer:req.headers.get("Referer")||"",fbclid:String(att.fbclid||"").slice(0,300),gclid:String(att.gclid||"").slice(0,300)},security:{quality_score:score,turnstile:true,honeypot:honey,ip_hash:await sha(ip||"unknown"),user_agent_hash:await sha(ua||"unknown"),reasons}};
