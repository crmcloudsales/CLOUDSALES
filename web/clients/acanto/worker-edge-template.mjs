...
export default{async fetch(req,env){
 ...
 if(req.method==='GET'&&path==='/health')return json({ok:true,service:'acanto-condo-gateway',version:'edge-1',challenge:'pow-hmac+turnstile',turnstile:'cloudflare-managed',seo:'indexable'});
 ...
 const proof=await sha(`${id}.${nonce}.${ts}.${sig}.${n}`);if(!proof.startsWith('000'))return json({message:'We could not validate the request.'},422);
 ...
 if(first.length<2||phone.replace(/\D/g,'').length<8||!validEmail(em))return json({message:'Please check your name, phone number and email.'},422);
 let score=35;const reasons=['edge_pow_hmac','cloudflare_turnstile'];score+=15;score+=20;score+=10;const dwell=Date.now()-Number(b.started_at||Date.now());if(dwell>=1800)score+=5;else reasons.push('fast_submit');if(honey){score=0;reasons.push('honeypot')}
 const att=b.attribution||{};
 const payload={gate_key:GATE_KEY,idempotency_key:id,origin_url:ORIGIN,form_id:'acanto-condominium-inquiry',form_name:'Acanto Condominiums · Current inventory request',form_answers:{interest,message},contact:{first_name:first,last_name:last,email:em,phone},attribution:{source_provider:clean(att.utm_source||'direct',80),campaign_id:clean(att.utm_campaign||'acanto-main',180),ad_group_id:clean(att.utm_content||interest,180),ad_id:'',landing_url:ORIGIN,referrer:req.headers.get('Referer')||'',fbclid:clean(att.fbclid,300),gclid:clean(att.gclid,300)},security:{quality_score:Math.max(0,Math.min(100,score)),turnstile:turnstileOk,honeypot:honey,ip_hash:await sha(ip||'unknown'),user_agent_hash:await sha(ua||'unknown'),reasons}};
