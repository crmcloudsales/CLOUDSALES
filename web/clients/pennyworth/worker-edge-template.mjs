...
form.onsubmit=async e=>{e.preventDefault();clearStatus();if(!form.reportValidity())return;const first=document.getElementById('first').value.trim(),last=document.getElementById('last').value.trim(),phone=document.getElementById('phone').value.trim(),email=document.getElementById('email').value.trim(),deliveryValue=document.getElementById('delivery')?.value||'',interestValue=document.getElementById('interest').value,website=document.getElementById('website').value;if(!first||!email||phone.replace(/\\D/g,'').length<8){show('Revisa tu nombre y teléfono para continuar.',false);return}send.disabled=true;send.textContent='VALIDANDO…';const id=crypto.randomUUID();try{const ch=await challenge(id),a=qp();send.textContent='ENVIANDO…';const channel=intent==='chat'?'chat':intent==='whatsapp'?'whatsapp':'landing_form',formId='pennyworth_shared_'+channel+'_v2';const r=await fetch('/lead',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({first_name:first,last_name:last,phone,email,website,started_at:intent==='landing'?Math.min(Date.now()-800,intentStarted):intentStarted,idempotency_key:id,challenge:ch,form_id:formId,form_name:'PENNYWORTH shared lead form',form_answers:{channel,interest:interestValue,delivery_timeline:deliveryValue,distribution_target:'listia_subscriber_pool'},attribution:{...a,property_interest:interestValue},notes:'Property interest: '+interestValue+(deliveryValue?' | Delivery: '+deliveryValue:'')})}),d=await r.json().catch(()=>({}));if(!(r.ok&&d.status==='accepted')){show(d.message||'No pudimos enviar la solicitud. Intenta nuevamente.',false);return}if(intent==='chat'){if(!d.chat_grant)throw new Error('secure_chat_grant_missing');show('Datos validados. Iniciando chat seguro…',true);await startChat(d.chat_grant,interestValue,deliveryValue);form.reset()}else if(intent==='whatsapp'){show('Datos validados. Abriendo WhatsApp…',true);const text='Hola, ya llené el formulario seguro en PENNYWORTH y me gustaría recibir información.\\n\\nNombre: '+first+(last?' '+last:'')+'\\nTeléfono: '+phone+(email?'\\nEmail: '+email:'')+'\\nInterés: '+interestValue+(deliveryValue?'\\nFecha de entrega: '+deliveryValue:'');form.reset();setTimeout(()=>{location.href='https://api.whatsapp.com/send?phone='+WHATSAPP+'&text='+encodeURIComponent(text)},250)}else{form.reset();show('Gracias. Tu solicitud fue recibida correctamente.',true)}}catch(err){show(err?.message==='secure_chat_grant_missing'?'No pudimos iniciar el chat seguro. Intenta nuevamente.':'No pudimos validar la solicitud. Intenta nuevamente.',false)}finally{send.disabled=false;if(intent==='chat')send.textContent='INICIAR CHAT';else if(intent==='whatsapp')send.textContent='CONTINUAR A WHATSAPP';else send.textContent=original.button}};
})();</script>`;
...
export default{async fetch(req,env){
 ...
 if(isShared&&(!em||!validEmail(em))){score=0;reasons.push("required_email_missing_or_invalid")}
 ...
 const payload={
   ...
   form_answers:isGate?{channel:clean(answers.channel,40),interest,message,consent:true,distribution_target:"listia_subscriber_pool"}:isShared?{channel:clean(answers.channel,40),interest,delivery_timeline:delivery,distribution_target:"listia_subscriber_pool"}:(b.form_answers||null),
   attribution:{
