...
function injectTurnstileClient(html:string,sitekey:string){
 let x=html.split("__TURNSTILE_SITEKEY__").join(sitekey);if(!x.includes('id="leadForm"')&&!x.includes("id='leadForm'"))return x;
 if(!x.includes('challenges.cloudflare.com/turnstile')){const api='<script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script>';const p=x.lastIndexOf('</head>');x=p>=0?x.slice(0,p)+api+x.slice(p):api+x}
 const native=/class=["'][^"']*cf-turnstile|turnstile\.render|data-sitekey=["'][^"']+/i.test(x);if(!native){const widget=`<div class="cf-turnstile" data-sitekey="${sitekey}" data-theme="auto" data-appearance="always" style="margin:10px 0;min-height:65px"></div>`;x=x.replace(/(<button\b[^>]*type=["']submit["'][^>]*>)/i,widget+'$1')}
 if(!x.includes('data-cloudsales-turnstile-bridge="v1"')){const bridge=`<script data-cloudsales-turnstile-bridge="v1">(()=>{'use strict';const of=window.fetch.bind(window);window.fetch=async function(input,init){const url=typeof input==='string'?input:(input&&input.url)||'';try{const u=new URL(url,location.href);if(u.origin===location.origin&&u.pathname==='/lead'&&init&&typeof init.body==='string'){const b=JSON.parse(init.body),t=String(b.turnstile_token||document.querySelector('[name="cf-turnstile-response"]')?.value||'');if(t)b.turnstile_token=t;init=Object.assign({},init,{body:JSON.stringify(b)})}}catch{}return of(input,init)}})();</script>`;const p=x.lastIndexOf('</body>');x=p>=0?x.slice(0,p)+bridge+x.slice(p):x+bridge}
 return x
}
function ensureServerTurnstile(raw:string,sitekey:string){
 let x=raw.split("__TURNSTILE_SITEKEY__").join(sitekey);if(x.includes('siteverify')&&x.includes('TURNSTILE_SECRET'))return x;
 const legacy='const ip=req.headers.get("CF-Connecting-IP")||"",ua=req.headers.get("User-Agent")||"",honey=String(b.website||"").trim()!=="";';
 if(!x.includes(legacy))throw new Error('turnstile_server_contract_missing');
 ...
 x=x.replace(legacy,verify);
 x=x.replace('security:{quality_score:score,turnstile:true,honeypot:honey','security:{quality_score:score,turnstile:turnstileOk,honeypot:honey');
 x=x.replace('challenge:"pow",inventory:','challenge:"pow-hmac+turnstile",turnstile:"cloudflare-managed",inventory:');
 x=x.replace("script-src 'self' 'unsafe-inline';","script-src 'self' 'unsafe-inline' https://challenges.cloudflare.com;");
 x=x.replace("connect-src 'self' https://fkahaqprzgcimgyathqx.supabase.co;","connect-src 'self' https://challenges.cloudflare.com https://fkahaqprzgcimgyathqx.supabase.co; frame-src https://challenges.cloudflare.com;");
 return x
}
async function smoke(host:string,marker:string){try{const r=await fetch(`https://${host}/?qa=${Date.now()}`,{redirect:"follow",headers:{"cache-control":"no-cache"}});const t=await r.text();return{ok:r.ok&&t.includes(marker),status:r.status,marker:t.includes(marker),compact_form:t.includes('data-cloudsales-compact-lead-form="v1"'),turnstile:t.includes('challenges.cloudflare.com/turnstile')||t.includes('cf-turnstile'),length:t.length}}catch(e){return{ok:false,status:0,error:String(e)}}}
Deno.serve(async req=>{
 ...
 try{
  ...
  if(!htmlRaw.includes(marker))throw new Error("landing_contract_failed");if(!tmplRaw.includes("__HTML_JSON__"))throw new Error("worker_contract_failed");
  const html=injectCompactLeadForm(injectTurnstileClient(htmlRaw,turn.sitekey)),template=ensureServerTurnstile(tmplRaw,turn.sitekey),code=template.replace("__HTML_JSON__",JSON.stringify(html));
  if(!template.includes('siteverify')||!template.includes('TURNSTILE_SECRET'))throw new Error('turnstile_server_missing_after_prepare');
  await upload(t,worker,code,edgeToken,crypto.randomUUID()+crypto.randomUUID(),turn.secret);result.steps.worker={uploaded:true,secrets_bound:true,compact_form:true,turnstile_server:true};result.steps.domain=await attach(t,host,worker);
  let sm:any=null;for(let n=0;n<4;n++){await new Promise(r=>setTimeout(r,n===0?4000:2500));sm=await smoke(host,marker);if(sm.ok&&sm.compact_form&&sm.turnstile)break}result.steps.smoke=sm;if(!sm?.ok||!sm?.compact_form||!sm?.turnstile)throw new Error(`smoke_failed_${sm?.status||0}`);
  const{data:g}=await svc.from("gate_configs").select("config").eq("id",gateId).maybeSingle();await svc.from("gate_configs").update({status:"active",config:{...(g?.config||{}),turnstile:"active",turnstile_sitekey:turn.sitekey,provisioning:{...(g?.config?.provisioning||{}),seo:"active",landing:"active",turnstile:"active",custom_domain:"enabled",edge_challenge:"active",cloudflare_worker:worker,dns_public_verified:true,worker_runtime_verified:true,compact_lead_form:"v1",anti_bot_standard:"turnstile+honeypot+pow+server-validation",updated_at:new Date().toISOString()}}}).eq("id",gateId);
