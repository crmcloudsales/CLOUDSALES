...
async function smoke(host:string,marker:string){try{const r=await fetch(`https://${host}/?qa=${Date.now()}`,{redirect:"follow",headers:{"cache-control":"no-cache"}});const t=await r.text();return{ok:r.ok&&t.includes(marker),status:r.status,marker:t.includes(marker),length:t.length}}catch(e){return{ok:false,status:0,error:String(e)}}}
Deno.serve(async req=>{
 ...
 try{
  ...
  await upload(t,worker,code,edgeToken,crypto.randomUUID()+crypto.randomUUID(),turn.secret);result.steps.worker={uploaded:true};result.steps.domain=await attach(t,host,worker);
  await new Promise(r=>setTimeout(r,3000));const sm=await smoke(host,marker);result.steps.smoke=sm;if(!sm.ok)throw new Error(`smoke_failed_${sm.status}`);
  const{data:g}=await svc.from("gate_configs").select("config").eq("id",gateId).maybeSingle();await svc.from("gate_configs").update({status:"active",config:{...(g?.config||{}),provisioning:{seo:"active",landing:"active",turnstile:"active",custom_domain:"enabled",edge_challenge:"active",cloudflare_worker:worker,dns_public_verified:true,worker_runtime_verified:true,updated_at:new Date().toISOString()}}}).eq("id",gateId);
