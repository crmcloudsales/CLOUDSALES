...
function patchTelemetry(raw:string){let x=raw;
 x=x.replaceAll("appearance:'always'","appearance:'interaction-only'").replaceAll('appearance:"always"','appearance:"interaction-only"');
 x=x.replace("if(!f||document.getElementById('pwTurnstileBox'))return;","if(!f||f.querySelector('.cf-turnstile,.pwTurnstileMount')||document.getElementById('pwTurnstileBox'))return;");
 x=x.replaceAll('font-size:10px!important','font-size:12px!important').replaceAll('font-size:11px!important','font-size:12px!important');
 x=x.replace("box.style.cssText='margin:10px 0;min-height:65px'","box.style.cssText='margin:0;min-height:0'");
 return x
}
function patchTurnstileTemplate(raw:string){let x=raw;
 ...
 const sec='security:{quality_score:Math.max(0,Math.min(100,score)),turnstile:true,honeypot:honey';if(x.includes(sec))x=x.replace(sec,'security:{quality_score:Math.max(0,Math.min(100,score)),turnstile:turnstileOk,honeypot:honey');
 x=x.replace('const reasons=["edge_pow_hmac"];','const reasons=["edge_pow_hmac","cloudflare_turnstile"];');
 x=x.replace('challenge:"pow-hmac",inventory:','challenge:"pow-hmac+turnstile",turnstile:"cloudflare-managed",inventory:').replace('challenge:"light-pow-hmac",inventory:','challenge:"light-pow-hmac+turnstile",turnstile:"cloudflare-managed",inventory:');
 x=x.replaceAll("appearance:'always'","appearance:'interaction-only'").replaceAll('appearance:"always"','appearance:"interaction-only"');
 ...
 x=x.replace("const pennyworth_chat_ux_v3='active';","const pennyworth_chat_ux_v3='active',pennyworth_chat_friction_v4='active';");
 const gateStyleClose="</style><script>(()=>{'use strict';";
 if(!x.includes('data-pennyworth-chat-compact-v4')){
  const compact=`\n/* data-pennyworth-chat-compact-v4 */\n#pwUnifiedPanel{width:min(400px,calc(100vw - 16px))!important}\n.pwHead{padding:12px 14px 10px!important}.pwHead strong{font-size:18px!important}.pwHead span{font-size:12px!important;line-height:1.35!important;margin-top:2px!important}.pwClose{width:34px!important;height:34px!important;font-size:22px!important}\n.pwFormHost{padding:6px!important}.pwFormHost .form-card{padding:12px!important;border-radius:18px!important}.pwFormHost .form-card>h3,.pwFormHost .form-card>.sub{display:none!important}\n.pwFormHost .formgrid{grid-template-columns:1fr 1fr!important;gap:6px 8px!important}.pwFormHost .field.full{grid-column:1/-1!important}.pwFormHost #leadForm .field{margin-top:5px!important}.pwFormHost #leadForm label{font-size:12px!important;line-height:1.25!important;margin-bottom:3px!important}.pwFormHost #leadForm input,.pwFormHost #leadForm select{padding:8px 10px!important;min-height:42px!important;border-radius:11px!important;font-size:16px!important}.pwChoiceButton{padding:8px 10px!important;min-height:42px!important;font-size:16px!important}.pwChoiceOption{font-size:14px!important;padding:10px!important}.pwChoiceMenu{max-height:250px!important}.pwFormHost #leadForm .submit{margin-top:8px!important;padding:10px 14px!important;min-height:44px!important;font-size:15px!important}.pwFormHost .fine{font-size:12px!important;line-height:1.32!important;margin-top:8px!important}.pwTurnstileMount{min-height:0!important;margin:0!important}.pwPanelNotice{top:60px!important;font-size:12px!important}.pwPropertyPicked,.pwPropertyPicked b{font-size:12px!important}\n@media(max-width:620px){#pwUnifiedPanel{left:8px!important;right:8px!important;top:8px!important;bottom:136px!important;width:auto!important}.pwHead{padding:11px 13px 9px!important}.pwFormHost{padding:5px!important}.pwFormHost .form-card{padding:10px!important}.pwFormHost #leadForm .field{margin-top:4px!important}.pwFormHost #leadForm input,.pwFormHost #leadForm select,.pwChoiceButton{min-height:40px!important;padding:7px 10px!important}.pwFormHost #leadForm .submit{margin-top:7px!important;min-height:42px!important}.pwFormHost .fine{margin-top:6px!important}}\n`;
  if(!x.includes(gateStyleClose))throw new Error('chat_style_close_missing');x=x.replace(gateStyleClose,compact+gateStyleClose);
 }
 x=x.replace(/if\(fine\)fine\.innerHTML='[^']*';/,"if(fine)fine.innerHTML='<strong>Condiciones de contacto:</strong> Al continuar autorizas el uso de tus datos para atender tu solicitud y, cuando corresponda, compartirlos con profesionales inmobiliarios relacionados. <a href=\"https://pennyworth.vip/privacy-policy\" target=\"_blank\" rel=\"noopener\">Aviso de privacidad</a>.';");
 return x
}
...
Deno.serve(async req=>{
 ...
 try{
  ...
  const liveHtml=await liveR.text(),rawTmpl=patchTurnstileTemplate(await tmplR.text()),telemetry=patchTelemetry(await telR.text()),html=injectTelemetry(stripRuntime(liveHtml),telemetry);
  result.steps.source={live_length:liveHtml.length,template_length:rawTmpl.length,telemetry_length:telemetry.length,telemetry_injected:html.includes('pennyworth-telemetry-v1'),turnstile_client:html.includes(TURNSTILE_SITEKEY),turnstile_server:rawTmpl.includes('TURNSTILE_SECRET'),turnstile_interaction_only:rawTmpl.includes("appearance:'interaction-only'"),chat_compact_v4:rawTmpl.includes('data-pennyworth-chat-compact-v4'),telemetry_duplicate_turnstile_guard:telemetry.includes("f.querySelector('.cf-turnstile,.pwTurnstileMount')")};
  ...
  if(!rawTmpl.includes('pennyworth_chat_ux_v3')||!rawTmpl.includes('pennyworth_chat_friction_v4')||!rawTmpl.includes('ensureTurnstileToken')||!rawTmpl.includes('pwChoiceButton'))throw new Error('chat_ux_v4_missing');
  if(!rawTmpl.includes("appearance:'interaction-only'")||!rawTmpl.includes('data-pennyworth-chat-compact-v4'))throw new Error('low_friction_contract_missing');
  ...
  await svc.from('gate_configs').update({status:'active',hostname:HOST,allowed_origins:[`https://${HOST}`],updated_at:now,config:{...cfg,turnstile:'active',turnstile_sitekey:TURNSTILE_SITEKEY,contact_gates:{chat:'shared_required_form',whatsapp:'shared_required_form',form_strategy:'single_physical_form',anti_bot:'turnstile_interaction_only+honeypot+rate_limit+server_validation+light_pow',chat_entry:'accepted_lead_hmac_one_time_grant',distribution_target:'listia_subscriber_pool',minimum_font_px:12,chat_form:'compact_v4'},provisioning:{...(cfg.provisioning||{}),landing:'active',edge_challenge:'active',localization:'active',seo:'active',analytics:'active_first_party_posthog',cloudflare_worker:WORKER,highlevel:'connected',meta:'direct_capi_active',contact_form:'shared-single-form',secure_chat:'hmac-one-time',turnstile:'interaction-only',chat_form:'compact_v4',minimum_font_px:12,updated_at:now}}}).eq('id',GATE_ID);
  await new Promise(r=>setTimeout(r,9000));
  const[s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13]=await Promise.all([smoke(`https://${HOST}/`,'pennyworth-gateway-v1','content="noindex'),smoke(`https://${HOST}/health`,'"cloudflare-managed"'),smoke(`https://${HOST}/challenge?id=qa-${Date.now()}`,'nonce'),smoke(`https://${HOST}/`,'pwUnifiedPanel'),smokeSingleForm(`https://${HOST}/`),smoke(`https://${HOST}/`,'chat_grant'),smoke(`https://${HOST}/`,'pennyworth-i18n-v1'),smoke(`https://${HOST}/robots.txt`,'Sitemap: https://pennyworth.cloudsales.app/sitemap.xml'),smoke(`https://${HOST}/sitemap.xml`,'https://pennyworth.cloudsales.app/'),smoke(`https://${HOST}/`,'pennyworth-telemetry-v1'),smoke(`https://${HOST}/`,TURNSTILE_SITEKEY),smoke(`https://${HOST}/`,'pennyworth_chat_friction_v4'),smoke(`https://${HOST}/`,'data-pennyworth-chat-compact-v4')]);
  result.steps.smoke={page:s1,health:s2,challenge:s3,unified_panel:s4,single_form:s5,chat_grant:s6,i18n:s7,robots:s8,sitemap:s9,telemetry:s10,turnstile:s11,friction_v4:s12,compact_v4:s13};if(!Object.values(result.steps.smoke).every((x:any)=>x.ok))throw new Error('smoke_failed');
  await svc.from('audit_log').insert({organization_id:ORG_ID,actor_type:'system',action:'pennyworth.security_analytics.provisioned',entity_type:'gate_config',entity_id:GATE_ID,success:true,context:{hostname:HOST,worker:WORKER,analytics:'first_party_plus_posthog',turnstile:'cloudflare_managed_interaction_only_siteverify',pow_hmac:true,honeypot:true,rate_limit:true,direct_capi:true,chat_form:'compact_v4',minimum_font_px:12}});
