...
function assertContract(x:string){
  const checks={intake:x.includes('/functions/v1/lead-intake'),no_turnstile_required:!x.includes('turnstile_required'),no_appearance_always:!/(?:appearance\s*:\s*['"]always['"]|data-appearance=["']always["'])/i.test(x),turnstile_signal:/turnstile:\s*turnstileOk/.test(x),review_continue:x.includes("d.status==='review'")&&x.includes('reviewContinue'),review_grant:x.includes('d?.status==="review"'),no_listia:!x.includes('listia_subscriber_pool')&&!x.includes('listia-subscriber-distribution')};
